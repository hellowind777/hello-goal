...
全局 API 恢复 + 混合守护插件。API 错误（socket 断开、429、502、503）无论是否 /goal 模式均自动恢复。在 /goal 模式下，额外通过行为结构分析 + LLM 语义分析防止任务非正常终止。语言无关，零外部依赖。纯 Python 标准库。
...
此外，**API 错误**（第三方大模型 socket 断开、429/502/503）会中断**任何**任务——无论是否 /goal 模式。hello-goal v2.3.1 在全局 Phase 0 优先拦截，自动恢复。

**hello-goal v2.3.1** 以单一 command-type Stop hook 实现四层级联守护。所有 BLOCK reason 精简为 `"继续"`，不产生冗长诊断信息干扰 AI 推理。

### v2.3 vs v2.2

| | v2.2 | v2.3 |
|---|---|---|
| JSON 输出 | `sys.stdout.buffer.write()` + `sys.stdout.flush()` | `os.write(fd=1)` — 直接写文件描述符，彻底绕过 Windows 编码层 |
| stderr | 未管控 — Python 警告可能污染 hook 输出 | 启动时重定向到 `os.devnull` |
| 进程退出 | `sys.exit(0)` — 可能触发 atexit 回调 | `os._exit(0)` — 立即退出，零额外输出 |
...
| JSON 兜底 | 1 层 | 3 层: `os.write(1)` → `sys.stdout.buffer` → `print(ascii)` |
...
| 场景 | 无 hello-goal | 有 hello-goal v2.3.1 |
|------|-------------|------------------|
...
v2.3.1 的行为分析**不读文字内容**——只对 transcript 中的工具调用模式、消息长度趋势、轮次结构打分。这些信号在任何语言中完全相同。LLM 语义分析对所有可疑轮次做最终裁决，天然理解任意语言。
...
| `plugins/hello-goal/.claude-plugin/plugin.json` | 插件元数据（v2.3.1） |
...
**A:** 不需要。v2.3.1 通过 CC 原生标记从 transcript 自动检测 /goal 状态，不依赖提示词写入任何文件。
</details>
...
**A:** 主入口包含全局异常兜底，配 3 层 JSON 输出兜底（`os.write(1)` → `sys.stdout.buffer` → `print(ascii)`）。任何未预期的内部异常都输出合法的 BLOCK 决策让任务继续执行，不会因 Python 错误导致 "JSON validation failed" 中断任务。
</details>
